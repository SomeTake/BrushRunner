...
D3DXAnimation::D3DXAnimation(const char* tag)
{
	...
	// アニメーションコントローラをコピー
	AnimController = model->AnimController;
...
{
	float CurWeightTime = this->model->AnimSet.at(this->CurrentAnimID)->GetCurWeightTime();
	float ShiftTime = this->model->AnimSet.at(this->CurrentAnimID)->GetShiftTime();

	// 合成中か否かを判定
	CurWeightTime += Time;
	this->model->AnimSet.at(this->CurrentAnimID)->SetCurWeightTime(CurWeightTime);
...
void D3DXAnimation::ChangeAnim(UINT AnimID)
{
	...
	ID3DXAnimationSet* CurtAnimSet = this->model->AnimSet.at(this->CurrentAnimID)->GetAnimSetPtr();
	ID3DXAnimationSet* NextAnimSet = this->model->AnimSet.at(AnimID)->GetAnimSetPtr();
	float Speed = this->model->AnimSet.at(AnimID)->GetSpeed();

	// 指定のアニメーションIDの存在をチェック
	if (AnimID > (UINT)this->model->GetAnimSetNum())
	{
	...
	// ウェイト時間を初期化
	this->model->AnimSet.at(AnimID)->SetCurWeightTime(0.0f);
...
{
	return (int)(this->model->AnimSet.at(this->CurrentAnimID)->GetPeriod() * 60);
}
